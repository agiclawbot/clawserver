//! claw-cli 自带的最小内置工具：
//!
//! - 默认（`tools-builtin` feature 开）注册 `claw_core::tools::builtin::{TimeNow, HttpGet, WebSearch}`
//! - 始终注册一个简单的 [`Echo`]（cli-only 调试工具，无外部依赖）
//!
//! 设计：仅依赖 [`claw_core::tool`] 契约，cli 端可通过关闭 `tools-builtin` feature
//! 退化成最小 Echo 工具集。
...
impl Tool for Echo {
    ...
    async fn invoke(&self, args: Value) -> AppResult<String> {
        let text = args
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                claw_core::error::AppError::BadRequest(
...
/// 构造调试用默认工具注册表。
///
/// - `tools-builtin` 开启时：注册 `claw_core::tools::builtin::{TimeNow, HttpGet, WebSearch}`
...
pub fn build_default_registry() -> Arc<ToolRegistry> {
    ...
    {
        reg.register(Arc::new(claw_core::tools::builtin::TimeNow));
        reg.register(Arc::new(claw_core::tools::builtin::HttpGet::new()));
        reg.register(Arc::new(claw_core::tools::builtin::WebSearch));
    }
